...
const frag = /* glsl */ `
  ...
  const vec3 baseColor1 = vec3(0.427451, 0.160784, 0.850980); // vivid purple (#6d28d9)
  const vec3 baseColor2 = vec3(0.231373, 0.509804, 0.960784); // rich blue (#3b82f6)
  const vec3 baseColor3 = vec3(0.937255, 0.952941, 1.000000); // soft white (#eef2ff)
...
export default function Orb({
  ...
  const disablePointerRef = useRef(disablePointer);
  ...
  useEffect(() => {
    disablePointerRef.current = disablePointer;
  }, [disablePointer]);
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      const drivenHover = Math.max(0, Math.min(1, driveRef.current));
      const pointerHover = forceHoverRef.current ? 1 : targetHover;
      const effectiveHover = Math.max(drivenHover, pointerHover);
