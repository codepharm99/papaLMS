...
type ActionResult = { type: Action["type"]; ok: boolean; detail: string; data?: unknown; error?: string };

const ACTION_MODEL = "gpt-oss:20b";
const MAX_ACTIONS = 10;

const SYSTEM_PROMPT = `
You are an LMS teacher assistant. Convert the teacher's request into a JSON plan.
...
Supported actions:
- update_student: { "type": "update_student", "username": "<student login>", "name"?: "<new display name>" }
- upsert_weekly_score: {
    "type": "upsert_weekly_score",
...
Rules:
- Do not propose role changes. Only rename students, add grades, or create tests.
- Extract only what the teacher provided; do not invent course codes or usernames.
- If required fields are missing, return an empty "actions" array with a brief reason.
- Limit to ${MAX_ACTIONS} actions. Output JSON only (no markdown).
`.trim();
...
const clampCount = (n: number, min: number, max: number) => Math.min(Math.max(n, min), max);

function heuristicCreateTest(message: string): Action[] {
  const lower = message.toLowerCase();
  if (!lower.includes("тест") && !lower.includes("test")) return [];

  const titleMatch = message.match(/тест\s+([^\n,.!?]+)/i);
  let rawTitle = titleMatch?.[1]?.trim() ?? "";
  rawTitle = rawTitle.replace(/^(на|про)\s+/i, "").trim();
  if (rawTitle.includes("с ")) rawTitle = rawTitle.split("с ")[0]?.trim() || rawTitle;
  if (rawTitle.includes("по ")) rawTitle = rawTitle.split("по ")[0]?.trim() || rawTitle;
  if (!rawTitle) rawTitle = message.slice(0, 80).trim();

  const topicMatch = message.match(/про\s+([^.?!\n]+)/i) || message.match(/по\s+([^.?!\n]+)/i);
  const topic = topicMatch?.[1]?.trim() || rawTitle || "тему";

  const countMatch = message.match(/(\d+)\s+вопрос/i);
  const count = countMatch ? clampCount(parseInt(countMatch[1] || "0", 10) || 0, 1, 10) : 0;

  const questions =
    count > 0
      ? Array.from({ length: count }).map((_, idx) => ({
          text: `Вопрос ${idx + 1} про ${topic}`,
        }))
      : undefined;
  ...
  return [
    {
      type: "create_test",
      title: rawTitle || `Тест ${topic}`.slice(0, 120),
      questions,
    } satisfies CreateTestAction,
  ];
}
  ...
  try {
    ...
    if (parsed && typeof parsed === "object") {
      if ("plan" in parsed && parsed.plan && typeof parsed.plan === "object") {
        const nested = parsed.plan as Plan;
        return nested;
      }
...
async function handleUpdateStudent(action: UpdateStudentAction, teacherId: string): Promise<ActionResult> {
  ...
  const linked = await prisma.enrollment.findFirst({
    where: { userId: student.id, course: { teacherId } },
  });
  if (!linked) {
    return { type: action.type, ok: false, detail: "Student is not in your courses", error: "NOT_ENROLLED_WITH_TEACHER" };
  }

  const name = pickString(action.name);
  if (!name) return { type: action.type, ok: true, detail: "No name change provided", data: { id: student.id } };

  const updated = await prisma.user.update({ where: { id: student.id }, data: { name } });
  return {
    type: action.type,
    ok: true,
    detail: `Renamed ${updated.username} to ${updated.name}`,
    data: { id: updated.id, username: updated.username, name: updated.name },
  };
}

async function handleWeeklyScore(action: UpsertWeeklyScoreAction, teacherId: string): Promise<ActionResult> {
  ...
  const course = await prisma.course.findFirst({ where: { code: courseCode, teacherId } });
  if (!course) {
    return { type: action.type, ok: false, detail: `Course ${courseCode} not found for this teacher`, error: "COURSE_NOT_FOUND" };
  }

  const res = await setWeeklyScoreForTeacher(teacherId, {
    courseId: course.id,
...
async function handleCreateTest(action: CreateTestAction, teacher: { id: string; name: string; username: string; role: Role }): Promise<ActionResult> {
  const title = pickString(action.title);
  if (!title) return { type: action.type, ok: false, detail: "Title is required", error: "TITLE_REQUIRED" };

  const created = await createTestForTeacher(teacher, { title, description: pickString(action.description) || undefined });
  if ("error" in created) {
  ...
  return {
    type: action.type,
    ok: true,
    detail: `Created test "${created.item.title}"${added ? ` (${added} questions)` : ""}`,
...
export async function POST(req: Request) {
  ...
  let actions = Array.isArray(plan?.actions) ? plan!.actions.slice(0, MAX_ACTIONS) : [];

  if ((!plan || actions.length === 0) && message) {
    actions = heuristicCreateTest(message);
  }

  if (!plan && actions.length === 0) {
    return NextResponse.json({ ok: false, error: "MODEL_PLAN_FAILED", raw }, { status: 200 });
  }
  ...
  for (const action of actions) {
    if (action.type === "update_student") {
      results.push(await handleUpdateStudent(action, me.id));
    } else if (action.type === "upsert_weekly_score") {
      results.push(await handleWeeklyScore(action, me.id));
    } else if (action.type === "create_test") {
      results.push(await handleCreateTest(action, { id: me.id, name: me.name, username: me.username, role: me.role }));
    } else {
