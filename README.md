...
- Переменные окружения (сервер):
  ...
  - `UNSPLASH_ACCESS_KEY` — ключ для подстановки картинок из Unsplash в слайды (опционально).
...
- Если задан `UNSPLASH_ACCESS_KEY`, изображения подставляются через hotlink на оригинальный URL Unsplash, с обязательной атрибуцией и регистрацией загрузки через `download_location` (требование Unsplash API).
